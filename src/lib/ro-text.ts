// Parser del markup del cliente de RO (items y skills). El ÚNICO tag de
// corchetes que interpretamos es el color, `[color=RRGGBB]texto[/color]`; TODO
// lo demás entre corchetes ([Costume], [Event], [Stylist], nombres de
// skill/MVP…) es TEXTO a mostrar y se deja tal cual (no son tags). Además se
// limpian los tags de ÁNGULO de navegación (<NAVI>…</NAVI>, <INFO>…</INFO>,
// <TIPBOX>). Un color 000000 = "volver al color por defecto" (en el cliente es
...
// Solo el color es un tag; el resto de corchetes es texto (ver cabecera).
const COLOR_RE = /\[(\/?)color(?:=([0-9a-fA-F]{6}))?\]/gi;

// Quita los tags de ángulo de navegación del cliente: <INFO>destino</INFO>
// lleva las coordenadas (fuera con su contenido) y el resto (<NAVI>, </NAVI>,
// <TIPBOX>…) se elimina dejando el texto interior (la etiqueta del enlace). El
// patrón exige nombre de tag alfabético para no tocar un "<" o ">" sueltos.
function stripClientTags(line: string): string {
  return line.replace(/<INFO>[\s\S]*?<\/INFO>/gi, "").replace(/<\/?[A-Za-z]+>/g, "");
}

export function parseBBCode(rawLine: string): ColorSegment[] {
  const line = stripClientTags(rawLine);
